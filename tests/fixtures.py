...
from repeated_prisoners_dilemma.strategy import ConstantCooperationStrategy, ConstantDefectionStrategy
...
def cooperating_player_1():
    player = Player(strategy=ConstantCooperationStrategy(), number=PlayerNumber.PLAYER_1)
    return player


@pytest.fixture
def defecting_player_2():
    player = Player(strategy=ConstantDefectionStrategy(), number=PlayerNumber.PLAYER_2)
    return player
